...
function ExecSQL {
    param (
        ...
        [string] $database = "CargasEnergy"
    )

    <# Envirnoment Variables #>
    $Server = "10.210.3.1"              #Update Server IP
    $DB = $database                     #Test database name
    $u = "XX"							#Fill in username
    $p = "XXXXXXXXX"					#Fill in password
    ...
    $GetTables = ExecSQL "
                ...
                and c.table_name not like '%CargasQuery_%'
                and c.table_name not like '%_[0-9]%'
                and c.table_name not like '%SPI_%'
...
#    '<html><head><title>' + $GetDBName + ' (' + $GetVersion + ')</title><style>' + $style + '</style></head><body>' + $Report + '</body></html>' | Out-File $GetDBName"_DD2.html"
    '<html><head><title>' + $title + '</title><style>' + $style + '</style></head><body>' + $Report + '</body></html>' | Out-File $GetDBName"_DD.html"
